...
class AddSwitchToNearestCommand(private val plugin: Crypt) : CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        sender.sendMessage("§a[Crypt] Added switch '$switchName' to ${nearest.type} '${nearest.name}' (${String.format("%.1f", nearest.distance)} blocks away)")
        return true
    }
...
class RemoveSwitchFromNearestCommand(private val plugin: Crypt) : CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return true }
        ...
        if (nearest.activeSwitches.remove(switchName)) {
            plugin.interactableManager.save()
            sender.sendMessage("§a[Crypt] Removed switch '$switchName' from ${nearest.type} '${nearest.name}'")
        } else {
...
class ListSwitchesOnNearestCommand(private val plugin: Crypt) : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return true }
