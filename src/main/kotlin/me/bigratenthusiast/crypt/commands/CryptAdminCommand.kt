...
import me.bigratenthusiast.crypt.util.toConfigString
...
class CryptAdminCommand(private val plugin: Crypt) : CommandExecutor, TabCompleter {
    ...
    private fun handleSet(sender: CommandSender, args: Array<out String>) {
        ...
        if (args.size < 2) {
            sender.sendMessage("§cUsage: /crypt set <${plugin.configManager.validLocationKeys.joinToString("|")}>")
            return
        }
        ...
        sender.sendMessage("§a[Crypt] Set '$key' to ${sender.location.toConfigString()}")
    }

    private fun handleSpawn(sender: CommandSender, args: Array<out String>) {
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt spawn <add|remove|list>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
                plugin.configManager.addSpawnPoint(sender.location)
                val count = plugin.configManager.getSpawnPoints().size
                sender.sendMessage("§a[Crypt] Added spawn point #${count - 1} at ${sender.location.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt spawn remove <index>"); return }
                ...
                if (plugin.configManager.removeSpawnPoint(index)) {
                    sender.sendMessage("§a[Crypt] Removed spawn point #$index")
                } else {
            ...
            else -> sender.sendMessage("§cUsage: /crypt spawn <add|remove|list>")
        }
    }
    ...
    private fun handleCancel(sender: CommandSender) {
        ...
        if (pending != null) {
            sender.sendMessage("§a[Crypt] Cancelled pending ${pending.type} registration for '${pending.name}'.")
        } else {
    ...
    private fun handleReload(sender: CommandSender) {
        ...
        sm.load()
        sm.initVillagers()
        sender.sendMessage("§a[Crypt] Config reloaded.")
    }
    ...
    private fun handleSwitch(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt switch <register|remove|list> <name> [material]"); return }
        when (args[1].lowercase()) {
            "list" -> {
            ...
            "register" -> {
                if (args.size < 4) { sender.sendMessage("§cUsage: /crypt switch register <name> <material>"); return }
                ...
                im.save()
                sender.sendMessage("§a[Crypt] Registered switch '$name' (${mat.name}) at ${loc.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt switch remove <name>"); return }
                val name = args[2].lowercase()
                if (im.switches.remove(name) != null) {
                    ...
                    if (usages.isNotEmpty()) {
                        sender.sendMessage("§e[Crypt] Warning: ${usages.size} interactable(s) still reference switch '$name': ${usages.joinToString(", ")}")
                    }
                    im.save()
                    sender.sendMessage("§a[Crypt] Removed switch '$name'.")
                } else {
                    sender.sendMessage("§cSwitch '$name' not found.")
                }
            }
            else -> sender.sendMessage("§cUsage: /crypt switch <register|remove|list>")
        }
    }
    ...
    private fun handleJumpPad(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt jumppad <add|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt jumppad add <name>"); return }
                val name = args[2].lowercase()
                im.jumpPads[name] = InteractableManager.JumpPadData(sender.location.block.location, mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added jump pad '$name' at ${sender.location.block.location.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt jumppad remove <name>"); return }
                val name = args[2].lowercase()
                if (im.jumpPads.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed jump pad '$name'.") }
                else sender.sendMessage("§cJump pad '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "jumppad", im.jumpPads)
            else -> sender.sendMessage("§cUsage: /crypt jumppad <add|remove|setswitch>")
        }
    }
    ...
    private fun handleBounceStand(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt bouncestand <add|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt bouncestand add <name>"); return }
                val name = args[2].lowercase()
                im.bounceStands[name] = InteractableManager.BounceStandData(sender.location, mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added bounce stand '$name' at ${sender.location.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt bouncestand remove <name>"); return }
                val name = args[2].lowercase()
                if (im.bounceStands.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed bounce stand '$name'.") }
                else sender.sendMessage("§cBounce stand '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "bouncestand", im.bounceStands)
            else -> sender.sendMessage("§cUsage: /crypt bouncestand <add|remove|setswitch>")
        }
    }
    ...
    private fun handleWaterArc(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt waterarc <add|pos1|pos2|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt waterarc add <name>"); return }
                val name = args[2].lowercase()
                val loc = sender.location.block.location
                im.waterArcs[name] = InteractableManager.WaterArcData(loc, loc.clone(), mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added water arc '$name'. Set pos1 and pos2.")
            }
            "pos1" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt waterarc pos1 <name>"); return }
                val name = args[2].lowercase()
                ...
                im.save()
                sender.sendMessage("§a[Crypt] Set pos1 for water arc '$name' to ${data.pos1.toConfigString()}")
            }
            "pos2" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt waterarc pos2 <name>"); return }
                val name = args[2].lowercase()
                val data = im.waterArcs[name] ?: run { sender.sendMessage("§cWater arc '$name' not found."); return }
                data.pos2 = sender.location.block.location
                im.save()
                sender.sendMessage("§a[Crypt] Set pos2 for water arc '$name' to ${data.pos2.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt waterarc remove <name>"); return }
                val name = args[2].lowercase()
                if (im.waterArcs.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed water arc '$name'.") }
                else sender.sendMessage("§cWater arc '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "waterarc", im.waterArcs)
            else -> sender.sendMessage("§cUsage: /crypt waterarc <add|pos1|pos2|remove|setswitch>")
        }
    }
    ...
    private fun handleLavaTrap(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt lavatrap <add|pos1|pos2|settrigger|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt lavatrap add <name>"); return }
                val name = args[2].lowercase()
                val loc = sender.location.block.location
                im.lavaTraps[name] = InteractableManager.LavaTrapData(loc, loc.clone(), loc.clone(), mutableListOf())
                im.save()
                sender.sendMessage("§a[Crypt] Added lava trap '$name'. Set pos1, pos2, and trigger.")
            }
            "pos1" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt lavatrap pos1 <name>"); return }
                val name = args[2].lowercase()
                val data = im.lavaTraps[name] ?: run { sender.sendMessage("§cLava trap '$name' not found."); return }
                data.pos1 = sender.location.block.location
                im.save()
                sender.sendMessage("§a[Crypt] Set pos1 for lava trap '$name' to ${data.pos1.toConfigString()}")
            }
            "pos2" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt lavatrap pos2 <name>"); return }
                val name = args[2].lowercase()
                val data = im.lavaTraps[name] ?: run { sender.sendMessage("§cLava trap '$name' not found."); return }
                data.pos2 = sender.location.block.location
                im.save()
                sender.sendMessage("§a[Crypt] Set pos2 for lava trap '$name' to ${data.pos2.toConfigString()}")
            }
            "settrigger" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt lavatrap settrigger <name>"); return }
                val name = args[2].lowercase()
                val data = im.lavaTraps[name] ?: run { sender.sendMessage("§cLava trap '$name' not found."); return }
                data.trigger = sender.location.block.location
                im.save()
                sender.sendMessage("§a[Crypt] Set trigger for lava trap '$name' to ${data.trigger.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt lavatrap remove <name>"); return }
                val name = args[2].lowercase()
                if (im.lavaTraps.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed lava trap '$name'.") }
                else sender.sendMessage("§cLava trap '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "lavatrap", im.lavaTraps)
            else -> sender.sendMessage("§cUsage: /crypt lavatrap <add|pos1|pos2|settrigger|remove|setswitch>")
        }
    }
    ...
    private fun handleSnake(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt snake <add|addpoint|clearpoints|setspeed|setlength|setblocks|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt snake add <name>"); return }
                val name = args[2].lowercase()
                im.snakes[name] = InteractableManager.SnakeData(mutableListOf(), 4, mutableListOf())
                im.save()
                sender.sendMessage("§a[Crypt] Added snake '$name'. Use /crypt snake addpoint $name to add route points.")
            }
            "addpoint" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt snake addpoint <name>"); return }
                val name = args[2].lowercase()
                ...
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added point #${data.points.size - 1} to snake '$name' at ${loc.toConfigString()}")
            }
            "clearpoints" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt snake clearpoints <name>"); return }
                val name = args[2].lowercase()
                val data = im.snakes[name] ?: run { sender.sendMessage("§cSnake '$name' not found."); return }
                plugin.snakeBlockTask.stopSnake(name)
                data.points.clear()
                im.save()
                sender.sendMessage("§a[Crypt] Cleared all points for snake '$name'.")
            }
            "setspeed" -> {
                if (args.size < 4) { sender.sendMessage("§cUsage: /crypt snake setspeed <name> <ticks>"); return }
                val name = args[2].lowercase()
                val data = im.snakes[name] ?: run { sender.sendMessage("§cSnake '$name' not found."); return }
                ...
                plugin.snakeBlockTask.stopSnake(name)
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Set snake '$name' speed to $ticks ticks per step.")
            }
            "setlength" -> {
                if (args.size < 4) { sender.sendMessage("§cUsage: /crypt snake setlength <name> <blocks>"); return }
                val name = args[2].lowercase()
                val data = im.snakes[name] ?: run { sender.sendMessage("§cSnake '$name' not found."); return }
                ...
                plugin.snakeBlockTask.stopSnake(name)
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Set snake '$name' length to $len blocks.")
            }
            "setblocks" -> {
                if (args.size < 4) { sender.sendMessage("§cUsage: /crypt snake setblocks <name> <block1> [block2] ..."); return }
                val name = args[2].lowercase()
                val data = im.snakes[name] ?: run { sender.sendMessage("§cSnake '$name' not found."); return }
                ...
                plugin.snakeBlockTask.stopSnake(name)
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Set snake '$name' blocks to: ${materials.joinToString(", ") { it.name }}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt snake remove <name>"); return }
                val name = args[2].lowercase()
                if (im.snakes.containsKey(name)) {
                    plugin.snakeBlockTask.stopSnake(name)
                    im.snakes.remove(name)
                    im.save()
                    sender.sendMessage("§a[Crypt] Removed snake '$name'.")
                } else {
                    sender.sendMessage("§cSnake '$name' not found.")
                }
            }
            "setswitch" -> handleSetSwitch(sender, args, "snake", im.snakes)
            else -> sender.sendMessage("§cUsage: /crypt snake <add|addpoint|clearpoints|setspeed|setlength|setblocks|remove|setswitch>")
        }
    }
    ...
    private fun handlePortal(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt portal <add|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt portal add <name>"); return }
                val name = args[2].lowercase()
                im.portals[name] = InteractableManager.PortalData(sender.location.block.location, mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added portal '$name' at ${sender.location.block.location.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt portal remove <name>"); return }
                val name = args[2].lowercase()
                if (im.portals.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed portal '$name'.") }
                else sender.sendMessage("§cPortal '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "portal", im.portals)
            else -> sender.sendMessage("§cUsage: /crypt portal <add|remove|setswitch>")
        }
    }
    ...
    private fun handlePondPortal(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt pondportal <add|pos1|pos2|addstop|clearstops|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt pondportal add <name>"); return }
                val name = args[2].lowercase()
                val loc = sender.location.block.location
                im.pondPortals[name] = InteractableManager.PondPortalData(loc, loc.clone(), mutableListOf(), mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added pond portal '$name'. Set pos1, pos2, and add stops.")
            }
            "pos1" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt pondportal pos1 <name>"); return }
                val name = args[2].lowercase()
                val data = im.pondPortals[name] ?: run { sender.sendMessage("§cPond portal '$name' not found."); return }
                data.pos1 = sender.location.block.location
                im.save()
                sender.sendMessage("§a[Crypt] Set pos1 for pond portal '$name' to ${data.pos1.toConfigString()}")
            }
            "pos2" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt pondportal pos2 <name>"); return }
                val name = args[2].lowercase()
                val data = im.pondPortals[name] ?: run { sender.sendMessage("§cPond portal '$name' not found."); return }
                data.pos2 = sender.location.block.location
                im.save()
                sender.sendMessage("§a[Crypt] Set pos2 for pond portal '$name' to ${data.pos2.toConfigString()}")
            }
            "addstop" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt pondportal addstop <name>"); return }
                val name = args[2].lowercase()
                val data = im.pondPortals[name] ?: run { sender.sendMessage("§cPond portal '$name' not found."); return }
                data.stops.add(sender.location)
                im.save()
                sender.sendMessage("§a[Crypt] Added stop #${data.stops.size - 1} to pond portal '$name'")
            }
            "clearstops" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt pondportal clearstops <name>"); return }
                val name = args[2].lowercase()
                val data = im.pondPortals[name] ?: run { sender.sendMessage("§cPond portal '$name' not found."); return }
                data.stops.clear()
                im.save()
                sender.sendMessage("§a[Crypt] Cleared all stops for pond portal '$name'.")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt pondportal remove <name>"); return }
                val name = args[2].lowercase()
                if (im.pondPortals.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed pond portal '$name'.") }
                else sender.sendMessage("§cPond portal '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "pondportal", im.pondPortals)
            else -> sender.sendMessage("§cUsage: /crypt pondportal <add|pos1|pos2|addstop|clearstops|remove|setswitch>")
        }
    }
    ...
    private fun handleBell(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt bell <add|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt bell add <name>"); return }
                val name = args[2].lowercase()
                im.bells[name] = InteractableManager.BellData(sender.location.block.location, mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added bell '$name' at ${sender.location.block.location.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt bell remove <name>"); return }
                val name = args[2].lowercase()
                if (im.bells.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed bell '$name'.") }
                else sender.sendMessage("§cBell '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "bell", im.bells)
            else -> sender.sendMessage("§cUsage: /crypt bell <add|remove|setswitch>")
        }
    }
    ...
    private fun handleCrystal(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt crystal <add|remove|setswitch> <name>"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt crystal add <name>"); return }
                val name = args[2].lowercase()
                im.crystals[name] = InteractableManager.CrystalData(sender.location.block.location, mutableListOf())
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Added crystal launcher '$name' at ${sender.location.block.location.toConfigString()}")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt crystal remove <name>"); return }
                val name = args[2].lowercase()
                if (im.crystals.remove(name) != null) { im.save(); sender.sendMessage("§a[Crypt] Removed crystal launcher '$name'.") }
                else sender.sendMessage("§cCrystal launcher '$name' not found.")
            }
            "setswitch" -> handleSetSwitch(sender, args, "crystal", im.crystals)
            else -> sender.sendMessage("§cUsage: /crypt crystal <add|remove|setswitch>")
        }
    }
    ...
    private fun handleGhast(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt ghast <setstable|setswitch> [switches...]"); return }
        when (args[1].lowercase()) {
            "setstable" -> {
                im.ghastStable = sender.location
                im.save()
                refreshInteractables()
                sender.sendMessage("§a[Crypt] Set ghast stable to ${sender.location.toConfigString()}")
            }
            "setswitch" -> {
                ...
                im.save()
                refreshInteractables()
                if (im.ghastSwitches.isEmpty()) sender.sendMessage("§a[Crypt] Ghast will be always active.")
                else sender.sendMessage("§a[Crypt] Ghast active on switches: ${im.ghastSwitches.joinToString(", ")}")
            }
            else -> sender.sendMessage("§cUsage: /crypt ghast <setstable|setswitch>")
        }
    }
    ...
    private fun handleKit(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt kit <edit|remove|list|status|giveuse|setcolor> [name]"); return }
        when (args[1].lowercase()) {
            "edit" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt kit edit <name>"); return }
                val name = args[2].lowercase()
                km.openEditGui(sender, name)
                sender.sendMessage("§a[Crypt] Editing kit '$name'. Close the GUI to save.")
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt kit remove <name>"); return }
                val name = args[2].lowercase()
                if (km.kits.remove(name) != null) {
                    km.save()
                    sender.sendMessage("§a[Crypt] Kit '$name' removed.")
                } else {
            ...
            "giveuse" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt kit giveuse <player>"); return }
                ...
                sender.sendMessage("§a[Crypt] Gave kit use to ${target.name}.")
                target.sendMessage("§a[Crypt] You can now use /kits again!")
            }
            "setcolor" -> {
                if (args.size < 4) { sender.sendMessage("§cUsage: /crypt kit setcolor <name> <color>"); return }
                val name = args[2].lowercase()
                ...
                km.save()
                sender.sendMessage("§a[Crypt] Kit '$name' shulker box color set to $color.")
            }
            else -> sender.sendMessage("§cUsage: /crypt kit <edit|remove|list|status|giveuse|setcolor>")
        }
    }
    ...
    private fun handleShop(sender: CommandSender, args: Array<out String>) {
        if (sender !is Player) { sender.sendMessage("§cOnly players can use this command."); return }
        if (args.size < 2) { sender.sendMessage("§cUsage: /crypt shop <add|edit|remove|setswitch|list> [name] [args...]"); return }
        when (args[1].lowercase()) {
            "add" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt shop add <name>"); return }
                val name = args[2].lowercase()
                ...
                sm.shops[name] = me.bigratenthusiast.crypt.shop.ShopManager.ShopData(
                ...
                sm.save()
                sender.sendMessage("§a[Crypt] Shop '$name' created with villager ${villager.uniqueId}")
                sender.sendMessage("§7Use /crypt shop edit $name to configure items and prices. Close GUI to save.")
            }
            "edit" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt shop edit <name>"); return }
                val name = args[2].lowercase()
                sm.openEditGui(sender, name)
            }
            "remove" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt shop remove <name>"); return }
                val name = args[2].lowercase()
                ...
                if (data != null) {
                    ...
                    sm.save()
                    sender.sendMessage("§a[Crypt] Shop '$name' removed. Villager restored to normal.")
                } else {
                    sender.sendMessage("§cShop '$name' not found.")
                }
            }
            "setswitch" -> {
                if (args.size < 3) { sender.sendMessage("§cUsage: /crypt shop setswitch <name> [switches...]"); return }
                val name = args[2].lowercase()
                ...
                sm.save()
                if (data.activeSwitches.isEmpty()) sender.sendMessage("§a[Crypt] Shop '$name' will be always active.")
                else sender.sendMessage("§a[Crypt] Shop '$name' active on switches: ${data.activeSwitches.joinToString(", ")}")
            }
            "list" -> {
            ...
            else -> sender.sendMessage("§cUsage: /crypt shop <add|edit|remove|setswitch|list>")
        }
    }
    ...
    private fun <T> handleSetSwitch(sender: CommandSender, args: Array<out String>, typeName: String, map: Map<String, T>) {
        if (args.size < 3) { sender.sendMessage("§cUsage: /crypt $typeName setswitch <name> [switches...]"); return }
        val name = args[2].lowercase()
        val entry = map[name]
        ...
        if (switchList.isEmpty()) sender.sendMessage("§a[Crypt] ${typeName.replaceFirstChar { it.uppercase() }} '$name' will be always active.")
        else sender.sendMessage("§a[Crypt] ${typeName.replaceFirstChar { it.uppercase() }} '$name' active on switches: ${switchList.joinToString(", ")}")
    }
    ...
    private fun sendHelp(sender: CommandSender) {
        sender.sendMessage("§6[Crypt] Admin Commands:")
        sender.sendMessage("§7  /crypt set <key> §8— set a named location")
        sender.sendMessage("§7  /crypt spawn <add|remove|list> §8— manage spawn points")
        sender.sendMessage("§7  /crypt switch <register|remove|list> §8— manage switches")
        sender.sendMessage("§7  /crypt jumppad <add|remove|setswitch> §8— jump pads")
        sender.sendMessage("§7  /crypt bouncestand <add|remove|setswitch> §8— bounce stands")
        sender.sendMessage("§7  /crypt waterarc <add|pos1|pos2|remove|setswitch> §8— water arcs")
        sender.sendMessage("§7  /crypt lavatrap <add|pos1|pos2|settrigger|remove|setswitch> §8— lava traps")
        sender.sendMessage("§7  /crypt snake <add|addpoint|clearpoints|setspeed|setlength|setblocks|remove|setswitch> §8— snakes")
        sender.sendMessage("§7  /crypt portal <add|remove|setswitch> §8— random teleporters")
        sender.sendMessage("§7  /crypt pondportal <add|pos1|pos2|addstop|clearstops|remove|setswitch> §8— pond portals")
        sender.sendMessage("§7  /crypt bell <add|remove|setswitch> §8— bells")
        sender.sendMessage("§7  /crypt crystal <add|remove|setswitch> §8— crystal launchers")
        sender.sendMessage("§7  /crypt ghast <setstable|setswitch> §8— happy ghast")
        sender.sendMessage("§7  /crypt kit <edit|remove|list|status|giveuse|setcolor> §8— kits (close GUI to save)")
        sender.sendMessage("§7  /crypt shop <add|edit|remove|setswitch|list> §8— shops (close GUI to save)")
        sender.sendMessage("§7  /crypt cancel §8— abort pending registration")
        sender.sendMessage("§7  /crypt reload §8— reload config")
    }
